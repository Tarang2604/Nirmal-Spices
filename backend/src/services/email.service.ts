...
export async function sendWelcomeEmail(email: string, name: string): Promise<void> {
  try {
    await resend.emails.send({
      ...
      to: email,
      reply_to: REPLY_TO,
      subject: "Welcome to Nirmal's Spices! ðŸŒ¶ï¸",
      html: `
        <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#FAF3E0;padding:32px;border-radius:16px">
          ...
          <p style="color:#3A3A3C;font-size:15px;line-height:1.7">
            Thank you for joining <strong>Nirmal's Spices</strong> â€” your gateway to 43 varieties of
...
// â”€â”€ OTP â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Order Confirmation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export async function sendOrderConfirmationEmail(
    ...
    .map(
      ...
      <tr>
        <td style="padding:10px 0;border-bottom:1px solid #E8E0D0">
          ...
          <span style="color:#8A8A8E;font-size:13px">${item.weight} Ã— ${item.qty}</span>
        </td>
  ...
  try {
    await resend.emails.send({
      ...
      to: email,
      reply_to: REPLY_TO,
      subject: `Order Confirmed! #${order._id} â€” Nirmal's Spices`,
      html: `
        <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#FAF3E0;padding:32px;border-radius:16px">
          ...
          <p style="color:#3A3A3C;font-size:15px">
            Thank you for your order. We'll deliver your spices in 3â€“5 business days.
          </p>
          <div style="background:white;border-radius:12px;padding:20px;margin:24px 0">
            ...
            <table style="width:100%;margin-top:16px;border-collapse:collapse">
              ...
              <tr>
                ...
                <td style="text-align:right;color:#8A8A8E">${order.shipping === 0 ? 'FREE' : `â‚¹${order.shipping}`}</td>
              </tr>
              ${order.discount > 0 ? `<tr><td style="padding:8px 0;color:#27AE60">Discount</td><td style="text-align:right;color:#27AE60">-â‚¹${order.discount}</td></tr>` : ''}
              <tr>
                <td style="padding:12px 0;font-weight:700;font-size:16px">Total</td>
                <td style="text-align:right;font-weight:800;font-size:16px;color:#C0392B">â‚¹${order.total.toLocaleString('en-IN')}</td>
              </tr>
            </table>
          </div>
...
// â”€â”€ Order Status Update â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export async function sendOrderStatusEmail(
  ...
  const statusMessages: Record<string, string> = {
    dispatched: `Your order has been dispatched! ðŸšš${trackingNumber ? ` Tracking: <strong>${trackingNumber}</strong>` : ''}`,
    'out-for-delivery': "Your order is out for delivery today! ðŸŽ‰",
    delivered: "Your order has been delivered. Enjoy your spices! ðŸŒ¶ï¸",
    cancelled: "Your order has been cancelled. Refund (if any) will be processed in 5â€“7 days.",
  };
  ...
  try {
    await resend.emails.send({
      from: FROM,
      to: email,
      subject: `Order Update: ${status.charAt(0).toUpperCase() + status.slice(1)} â€” Nirmal's Spices`,
      html: `
        <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#FAF3E0;padding:32px;border-radius:16px">
...
// â”€â”€ Newsletter Welcome â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export async function sendNewsletterWelcome(
  ...
  try {
    await resend.emails.send({
      from: FROM,
      to: email,
      subject: "You're subscribed to Nirmal's Spices! ðŸŒ¶ï¸",
      html: `
        <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#FAF3E0;padding:32px;border-radius:16px">
