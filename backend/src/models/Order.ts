...
export interface IOrder extends Document {
  ...
  guestEmail?: string;
  guestPhone?: string;
  guestSessionId?: string;          // guest cart session for webhook cleanup
  items: IOrderItem[];
  address: IAddress;
  ...
  platformFee: number;
  total: number;
  // Fee snapshot (what settings were at order time)
  feeSnapshot?: {
    commissionType: 'percent' | 'flat';
    commissionValue: number;
    platformFeeType: 'percent' | 'flat';
...
const orderSchema = new Schema<IOrder>(
  {
    ...
    feeSnapshot: {
      commissionType: { type: String, enum: ['percent', 'flat'] },
      commissionValue: { type: Number },
      platformFeeType: { type: String, enum: ['percent', 'flat'] },
...
// Auto-push to timeline when status changes (skip if caller already appended same status)
